# 👋 Hey — I’m Mohamed  
**Front-end Developer | Learner of Full-Stack Web Dev**  

![Header Banner](https://placehold.co/1200x240?text=Mohamed+Deeqsi+-+FrontEnd+Dev)

## 🚀 About Me  
- 🎓 IT student at Zamzam University  
- 💻 Passionate front-ende veloper: HTML, CSS, JavaScript  
- 🐍 Currently learning  full-stack skills  
- 📂 I build fun & practical small projects (portfolios, games, websites, etc.)  
- 🌍 From Somalia 

---

## 🧰 My Tech Stack  
![HTML](https://img.shields.io/badge/HTML5-%23E34F26.svg?logo=html5&logoColor=white)  
![CSS](https://img.shields.io/badge/CSS3-%231572B6.svg?logo=css3&logoColor=white)  
![JavaScript](https://img.shields.io/badge/JavaScript-%23F7DF1E.svg?logo=javascript&logoColor=black)  
  

---

## 📂 Highlighted Projects  
Here are a few projects I’m proud of — feel free to explore 👇  

- **Restaurant Website** — a responsive front-end project  
- **Library Management System** — a small admin-panel style system  
- **Rock-Paper-Scissors Game** — a basic game built with Python / JS  
- **Portfolio / Personal Site** — “my_portfolio” repo showcasing my skills  

---

## 📈 GitHub Stats  
![GitHub Stats Card](https://github-readme-stats.vercel.app/api?username=Mohamed-saYid-Mohamed&show_icons=true&theme=radical)  

---

## 🎯 What I’m Working On  
- Learning full-stack development: bridging front-end with backend (Django)  
- Building more interactive projects, real-world  
- Improving my code structure, documentation and design skills  

---

## 💬 Let’s Connect  
- 🌐 Portfolio / Live demo: _(add link if you have one)_  
- 📫 Email: _(your email)_  
- 🐙 GitHub: https://github.com/Mohamed-saYid-Mohamed  

---

*Built with ❤️ and curiosity — always learning.*  
